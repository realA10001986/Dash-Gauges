/*
  ...
  Adapted for loop-playback by Thomas Winischhofer (A10001986), 2023
...
bool AudioGeneratorWAVLoop::GetBufferedData(int bytes, void *dest)
{
  ...
  while (bytes--) {
    ...
    if (buffPtr >= buffLen) {
      buffPtr = 0;
      //uint32_t toRead = availBytes > buffSize ? buffSize : availBytes;  // TW
      //buffLen = file->read( buff, toRead );                             // TW
      //availBytes -= buffLen;                                            // TW
      buffLen = file->read( buff, buffSize );                             // TW
    }
    if (buffPtr >= buffLen)
